...
- **발표분 블록 내 3시간 평활** ([src/smooth.py](src/smooth.py)): 시간별 독립 예측의
  잡음을 인접 시간 평균으로 상쇄. 블록(01:00~익일 00:00) 내부로 엄격히 제한해
  누수를 차단한다. 이중 폴드 +0.0011/+0.0013, FICR 양쪽 상승.
- **v12 강규제 하이퍼파라미터**: `lr 0.02 / min_data_in_leaf 100 /
  feature_fraction 0.20 / lambda_l2 30`. v1(195피처) 기본값을 1577피처에서도
  쓰고 있던 것을 8개 설정 이중 폴드 탐색으로 교체했다. 1-NMAE 는 유지한 채
  **FICR 만 0.4140 → 0.4203** 으로 올라, Public 격차의 84% 를 차지하는 항목을
  정확히 겨냥한다.
