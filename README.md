...
## 접근 방법 (v4 — 2024 홀드아웃 총점 0.6377, 이중 폴드 검증)

v2/v3가 로컬 점수(0.6438/0.6457)에도 불구하고 Public에서 v1을 넘지 못한 교훈을 반영해,
**이중 폴드(22→23, 22-23→24) 모두에서 이기는 변경만 채택**하는 원칙으로 재설계.
...
- **SCADA 라벨 클리닝** (`src/scada.py`): SCADA 10분 에너지 합 = 라벨(corr 0.999)임을 확인,
  '자체 풍속 ≥ 5 m/s인데 무출력 터빈 ≥ 1대'인 정비/트립 시간대(그룹별 9~15%)를 학습에서
  제외 — 기상으로 설명 불가능한 라벨 노이즈 제거. 양 폴드 +0.013의 최대 정확도 레버.
...
- **누수 방지**: 제공 예보 데이터는 전일 13:00(KST) 발표분만 포함되어 있어
  ...
  SCADA는 학습 기간 라벨 클리닝에만 사용. 2025년 실측/사후 자료는 일절 미사용.

실험 과정 전체는 [docs/experiments.md](docs/experiments.md) 참고.
실험 재현: `python src/exp_runner.py stage1|stage2|stage3|stage4|stage5`
