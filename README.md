...
- **그룹3 통합학습**: 그룹3은 라벨이 2023년부터라 데이터가 절반 → 3그룹을 정규화 타깃
  (y/설비용량) + 그룹ID로 통합 학습 (그룹3 단독점수 0.58 → 0.61)
- **3시드 앙상블**: seed 42/202/777 평균
- **후처리 (scale, floor)**: 평가가 '실발전 ≥ 10%cap' 시간대만 대상이므로 낮은 예측값에
  하한(floor)을 두는 편이 유리(평가 제외 시간대는 무손실). v2 Public 제출에서 1-NMAE는
  전이됐지만 FICR이 낮아, 그룹1/2는 23·24 평균보다 덜 공격적이고 24년 점수가 낮지 않은
  24년 직접 튜닝값으로 조정. FICR 0.3750 → 0.4229의 최대 레버.
...
- **누수 방지**: 제공 예보 데이터는 전일 13:00(KST) 발표분만 포함되어 있어
  ...
  2025년 실측/사후 자료는 일절 미사용.

실험 과정 전체는 [docs/experiments.md](docs/experiments.md) 참고.
실험 재현: `python src/exp_runner.py stage1|stage2|stage3|stage4`
